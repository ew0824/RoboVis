"""
Enhanced Multi-URDF Viser system with coordinate frame visualization.
Displays RGB coordinate axes at every joint that update in real-time.

Launch:
    python benchmarks/viser_multi_urdf_with_frames.py --workcell workcell_alpha_2 --stress --stress-hz 200
"""
...
class CoordinateFrameManager:
    """Manages coordinate frame visualization for all joints."""
    
    def __init__(self, server: viser.ViserServer):
        self.server = server
        self.coordinate_frames: Dict[str, viser.FrameHandle] = {}
        self.show_frames = False
        self.frame_scale = 0.1
        
    def create_frame(self, name: str, position: np.ndarray, rotation: np.ndarray) -> None:
        """Create a coordinate frame at the specified pose."""
        frame_name = f"/frames/{name}"
        
        if frame_name in self.coordinate_frames:
            # Update existing frame
            self.coordinate_frames[frame_name].position = tuple(position)
            self.coordinate_frames[frame_name].wxyz = tuple(rotation)
        else:
            # Create new frame
            frame = self.server.scene.add_frame(
                frame_name,
                axes_length=self.frame_scale,
                axes_radius=self.frame_scale * 0.05,
                position=tuple(position),
                wxyz=tuple(rotation),
                visible=self.show_frames,
            )
            self.coordinate_frames[frame_name] = frame
    
    def update_frame_visibility(self, show: bool) -> None:
        """Toggle visibility of all coordinate frames."""
        self.show_frames = show
        for frame in self.coordinate_frames.values():
            frame.visible = show
    
    def update_frame_scale(self, scale: float) -> None:
        """Update the scale of all coordinate frames."""
        self.frame_scale = scale
        for frame in self.coordinate_frames.values():
            frame.axes_length = scale
            frame.axes_radius = scale * 0.05
...
class SmartUrdfManager:
    """Enhanced URDF manager with coordinate frame visualization."""
    ...
    def __init__(self, server: viser.ViserServer):
        ...
        self.frame_manager = CoordinateFrameManager(server)
    ...
    def _update_coordinate_frames(self, joint_values: np.ndarray) -> None:
        """Update coordinate frame positions based on current joint configuration."""
        if not self.frame_manager.show_frames:
            return
            
        # Temporarily store joint values for pose computation
        temp_joint_values = {}
        for i, joint_name in enumerate(self.filtered_joint_names):
            temp_joint_values[joint_name] = joint_values[i]
        
        # Compute poses for each URDF separately
        for config in self.urdf_configs:
            urdf_name = config["name"]
            urdf = config["urdf"]
            viser_urdf = config["viser_urdf"]
            ...
            try:
                # Build joint configuration for this URDF
                actuated_joints = viser_urdf.get_actuated_joint_limits()
                joint_angles = {}
                
                for joint_name in actuated_joints.keys():
                    unique_name = f"{urdf_name}::{joint_name}"
                    if unique_name in temp_joint_values:
                        joint_angles[joint_name] = temp_joint_values[unique_name]
                    else:
                        # Use default for auxiliary joints
                        lower, upper = actuated_joints[joint_name]
                        if lower is not None and upper is not None:
                ...
                # Update coordinate frames for joints
                for joint in urdf.robot.joints:
                    if joint.child in fk:
                        transform = fk[joint.child]
                        position = transform[:3, 3]
                        ...
                        frame_name = f"{urdf_name}_{joint.name}"
                        self.frame_manager.create_frame(frame_name, position, quat_wxyz)
                        
            except Exception as e:
                print(f"[FRAMES] Error updating frames for {urdf_name}: {e}")
    ...
    def update_all_configurations(self, joint_values: np.ndarray):
        ...
        self._update_coordinate_frames(joint_values)
...
def main(
    ...
    # Add coordinate frame controls
    with server.gui.add_folder("📐 Coordinate Frames"):
        show_frames_cb = server.gui.add_checkbox("Show coordinate frames", False)
        frame_scale_slider = server.gui.add_slider(
            "Frame scale",
            min=0.01,
            max=0.5,
            step=0.01,
        ...
        frame_info = server.gui.add_text("Info", "Toggle frames above to see joint coordinate systems")
    
    @show_frames_cb.on_update
    def _(_):
        urdf_manager.frame_manager.update_frame_visibility(show_frames_cb.value)
        if show_frames_cb.value:
            # Trigger frame update with current configuration
            current_cfg = np.array([s.value for s in slider_handles], dtype=np.float32)
            urdf_manager._update_coordinate_frames(current_cfg)
            frame_info.value = f"✅ Showing coordinate frames (scale: {frame_scale_slider.value:.2f})"
        else:
            frame_info.value = "❌ Coordinate frames hidden"
    
    @frame_scale_slider.on_update
    def _(_):
        urdf_manager.frame_manager.update_frame_scale(frame_scale_slider.value)
        if show_frames_cb.value:
            frame_info.value = f"✅ Showing coordinate frames (scale: {frame_scale_slider.value:.2f})"
    ...
    if stress:
        ...
        def run_stress_testing_in_thread():
            ...
            print(f"   Coordinate frames: {'✅ ON' if show_frames_cb.value else '❌ OFF'}")
            print()
            ...
            while stress_running.is_set():
                ...
                if loop_start - last_stats >= 10.0:
                    ...
                    frame_status = "📐 ON" if show_frames_cb.value else "❌ OFF"
                    print(f"🔥 [SMART-STRESS] {msg_count:,} updates | {elapsed:.1f}s | {avg_hz:.1f} Hz avg | Target: {hz:.1f} Hz | {nq}/{meaningful_dof} DOF | Frames: {frame_status}")
