"""
Fixed Multi-URDF Viser stress testing benchmark with intelligent path resolution.
Addresses all issues: mesh path resolution, URDF deduplication, joint filtering, and GUI layout.

Launch:
    python benchmarks/viser_multi_urdf_stress_test_fixed.py --workcell workcell_alpha_2 --stress --stress-hz 200
"""
...
class SmartUrdfManager:
    """Enhanced URDF manager with deduplication, joint filtering, and smart path resolution."""
...
def main(
    ...
    """
    Smart Multi-URDF stress testing with path resolution, deduplication, and joint filtering.
    """
    ...
    if stress:
        ...
        def run_stress_testing_in_thread():
            """Run smart stress testing loop with filtered joints."""
            print(f"🚀 [SMART-STRESS] Starting intelligent multi-URDF stress test:")
            print(f"   Total URDFs: {len(urdf_configs)}")
            print(f"   Meaningful DOF: {meaningful_dof}")
            ...
            while stress_running.is_set():
                ...
                if loop_start - last_stats >= 10.0:
                    ...
                    print(f"🔥 [SMART-STRESS] {msg_count:,} updates | {elapsed:.1f}s | {avg_hz:.1f} Hz avg | Target: {hz:.1f} Hz | {nq}/{meaningful_dof} DOF")
    ...
    print(f"\n[MAIN] 🎉 Smart Multi-URDF System Ready!")
    print(f"[MAIN] Ready for stress testing with {meaningful_dof} meaningful DOF")
