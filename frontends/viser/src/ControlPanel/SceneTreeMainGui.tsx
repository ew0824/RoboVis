...
/* Optimized Scene Tree for Main GUI with coordinate frame controls */
const SceneTreeMainGui = React.memo(function SceneTreeMainGui() {
  const viewer = React.useContext(ViewerContext)!;
...
export default SceneTreeMainGui;
...
// Enhanced SceneTreeTableRow with coordinate frame controls
const SceneTreeTableRow = React.memo(function SceneTreeTableRow(props: {
  ...
  // Check if this is a coordinate frame node
  const nodeMessage = viewer.useSceneTree(
    (state) => state.nodeFromName[props.nodeName]?.message,
  );
  const isCoordinateFrame = nodeMessage?.type === "FrameMessage";
  ...
  // Coordinate frame toggle handler
  const toggleCoordinateFrame = (evt: React.MouseEvent) => {
    evt.stopPropagation();
    
    if (isCoordinateFrame) {
      // For coordinate frames, toggle show_axes property
      const currentShowAxes = nodeMessage?.props?.show_axes || false;
      viewer.useSceneTree.getState().updateSceneNode(props.nodeName, {
        show_axes: !currentShowAxes,
      });
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* Coordinate frame toggle button with emoji */}
        {isCoordinateFrame && (
          <Box
          ...
          >
            <Tooltip label="Toggle coordinate frame">
              <span
                style={{
                  cursor: "pointer",
                  fontSize: "1.2em",
                  display: "block",
                  opacity: nodeMessage?.props?.show_axes ? 1.0 : 0.4,
                  filter: nodeMessage?.props?.show_axes 
                    ? `drop-shadow(0 0 2px ${theme.colors.blue[colorScheme === "dark" ? 4 : 6]})`
                    : undefined,
                }}
                onClick={toggleCoordinateFrame}
              >
                📐
              </span>
            </Tooltip>
